                                                      ...
                                                      where TFilaModel : IValidable
{
    ...
    public async Task ProcesarBloqueDeArchivoAsync(Guid idArchivoCarga, Guid idBloqueCarga)
    {
        ...
        bool readModel = true;
        bool readFilaModel = false;
        var esUltimo = await ProcesarBloque(objArchivoCarga, idBloqueCarga, readModel, readFilaModel);
        if (esUltimo)
        {
            await FinalizarProcesoDeBloquesDeArchivo(idArchivoCarga);
        }
    ...
    private async Task<bool> ProcesarBloque(ArchivoCarga objArchivoCarga, Guid guidBloque, bool readModel, bool readFilaModel)
    {
        ContadoresProceso contadoresBloque = null;
        TBloque objBloqueDeBD = null; //para pruebas de verificación;

        var objBloqueEnProceso = ObtenerActualBloqueAProcesar(guidBloque);
        if (objBloqueEnProceso == null) //Si ya no existen bloques que procesar
        {
            return false;
        }
        ...
        var esUltimoBloque = VerficarSiEsUltimoBloqueAProcesar(guidBloque);
